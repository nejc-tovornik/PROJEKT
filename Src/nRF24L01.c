...
static uint8_t nRF24_ReadReg(NRF *dev, uint8_t reg) {
	uint8_t rxData[1];
	...
	HAL_GPIO_WritePin(dev->CSN_GPIO_PORT, dev->CSN_PIN, GPIO_PIN_SET);
	return rxData[0];
}

static void nRF24_WriteReg(NRF *dev, uint8_t reg, uint8_t value) {
	uint8_t txData[2];
	HAL_GPIO_WritePin(dev->CSN_GPIO_PORT, dev->CSN_PIN, GPIO_PIN_RESET);
	if (reg < nRF24_CMD_W_REGISTER) {
		// This is a register access
		txData[0] = nRF24_CMD_W_REGISTER | (reg & nRF24_MASK_REG_MAP );
		txData[1] = value;
		...
		HAL_SPI_Transmit(dev->hspi, txData, 1, 100);
...
static void nRF24_WriteMBReg(NRF *dev, uint8_t reg, uint8_t *pBuf,
		uint8_t count) {
	HAL_GPIO_WritePin(dev->CSN_GPIO_PORT, dev->CSN_PIN, GPIO_PIN_RESET);
	HAL_SPI_Transmit(dev->hspi, &reg, 1, 100);
	HAL_SPI_Transmit(dev->hspi, pBuf, count, 100);
	HAL_GPIO_WritePin(dev->CSN_GPIO_PORT, dev->CSN_PIN, GPIO_PIN_SET);
}
