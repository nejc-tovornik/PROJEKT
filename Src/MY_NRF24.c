...
void NRF24_csn(int state) {
	if (state)
		HAL_GPIO_WritePin(nrf24_PORT_CSN, nrf24_CSN_PIN, GPIO_PIN_SET);
	else
		HAL_GPIO_WritePin(nrf24_PORT_CSN, nrf24_CSN_PIN, GPIO_PIN_RESET);
}
//2. Chip Enable
void NRF24_ce(int state) {
	if (state)
		HAL_GPIO_WritePin(nrf24_PORT_CE, nrf24_CE_PIN, GPIO_PIN_SET);
	else
		HAL_GPIO_WritePin(nrf24_PORT_CE, nrf24_CE_PIN, GPIO_PIN_RESET);
}
...
//12. Begin function
void NRF24_begin(GPIO_TypeDef *nrf24PORT_CSN, GPIO_TypeDef *nrf24PORT_CE,
		uint16_t nrfCSN_Pin, uint16_t nrfCE_Pin, SPI_HandleTypeDef nrfSPI) {
...
void NRF24_maskIRQ(bool tx_ok, bool tx_fail, bool rx_ready) {
	...
	config |= tx_fail << MASK_MAX_RT | tx_ok << MASK_TX_DS
			| rx_ready << MASK_RX_DR;
